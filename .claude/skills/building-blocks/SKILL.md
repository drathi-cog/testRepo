...
**Block directory:** Always create block files at `C:\Users\2464702\projects\boiler-plate\testRepo\blocks\{block-name}\{block-name}.js` and `.css`. Never create blocks in worktrees.

**CSS scoping:** Follow the existing repo convention — scope selectors to `.{block-name}` (e.g., `.callout`, `.news-feed`), not `main .{block-name}`. Keep selectors short and avoid deep nesting.

**JS patterns:** Export a default `decorate(block)` function. Use `block.querySelector(':scope > div')` to access rows. Re-use existing DOM nodes, do not recreate them from text content.
